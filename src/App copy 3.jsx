...
const EnhancedIslandSystem = () => {
  ...
  const anchorDataSourceRef = useRef(null); // New ref for anchor points
  ...
  const [anchorData, setAnchorData] = useState([]); // New state for anchor points
  ...
  const BACKEND_URL = 'http://192.168.6.225:8081/api/geojson'; // Backend endpoint
  ...
  const loadAnchorData = async () => {
    try {
      ...
      // Filter anchor points (assuming they have specific properties or are in a specific collection)
      const anchorFeatures = geoJsonData.features || [];
      setAnchorData(anchorFeatures);
      
      // Create data source for anchor points
      if (anchorFeatures.length > 0) {
        ...
        for (let i = 0; i < entities.length; i++) {
          ...
          // Add custom styling
          entity.billboard = {
            image: "/images/cctv-camera.png", // Your CCTV icon
            width: 32,
            height: 32,
        ...
        anchorDataSource.show = showBuildings; // Toggle with buildings
  ...
  const saveAnchorToBackend = async (anchorFeature) => {
    try {
      ...
      // Get current anchor data and add new feature
  ...
  // Updated handle map click with backend integration
